import React, { useState, useEffect } from "react";
import Navbar from "./components/Navbar";
import Footer from "./components/Footer";
import HomePage from "./pages/HomePage";

import AboutPage from "./pages/AboutPage";
import GalleryPage from "./pages/GalleryPage";
import TestimonialsPage from "./components/TestimonialsSection";
import Contact from "./pages/Contact";
import Programs from "./pages/Programs";
import Preloader from "./components/Preloader";
import AddTestimonials from "./components/AddTestimonials";
import JeePreparation from "./pages/JeePreparation";
import NEETPreparation from "./pages/NeetPreparation";
import FoundationClass from "./pages/FoundationClass";
import BoardPreparation from "./pages/BoardPreparation";

// Router Implementation
const Router = ({ children }) => {
  const [currentPath, setCurrentPath] = useState(window.location.pathname);

  useEffect(() => {
    const handlePopState = () => setCurrentPath(window.location.pathname);
    window.addEventListener('popstate', handlePopState);
    return () => window.removeEventListener('popstate', handlePopState);
  }, []);

  const navigate = (path) => {
    window.history.pushState({}, '', path);
    setCurrentPath(path);
  };

  return React.Children.map(children, child =>
    React.cloneElement(child, { currentPath, navigate })
  );
};

const Route = ({ path, component: Component, currentPath, navigate }) => {
  return currentPath === path ? <Component navigate={navigate} /> : null;
};

// Main App Component
const App = () => {
  const [isLoading, setIsLoading] = useState(true);

  const handleLoadComplete = () => {
    setIsLoading(false);
  };

  return (
    <>
      {isLoading && <Preloader onLoadComplete={handleLoadComplete} />}

      <div className={`transition-opacity duration-700 ${isLoading ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
        <Router>
          <Navbar />
          <Route path="/" component={HomePage} />
          <Route path="/programs" component={Programs} />
          <Route path="/about" component={AboutPage} />
          <Route path="/campus" component={GalleryPage} />
          <Route path="/testimonials" component={TestimonialsPage} />
          <Route path="/contact" component={Contact} />
          <Route path="/submit-testimonial" component={AddTestimonials} />
          <Route path="/jee-preparation" component={JeePreparation} />
          <Route path="/neet-preparation" component={NEETPreparation} />
          <Route path="/foundation-class" component={FoundationClass} />
          <Route path="/board-preparation" component={BoardPreparation} />
          <Footer />
        </Router>
      </div>
    </>
  );
};

export default App;